...
fn optionnal_field_with_separator(data: &[u8]) -> IResult<&[u8], (&[u8], &[u8], &[u8])> {
    ...
    println!("optionnal_field_with_separator {:?}", result);
...
pub fn parse_message_with_separators(
    data: &[u8],
) -> Result<(Vec<(&[u8], &[u8], &[u8])>, Option<&[u8]>), ()> {
    if let Ok(result) = tuple((fields_with_separators, opt(tuple((tag("\r\n"), body)))))(data) {
        println!("parse_message_with_separators {:?}", result);
        Ok(((result.1).0, (result.1).1.map(|b| b.1)))
    } else {
        println!("parse_message_with_separators error");
        Err(())
    }
}
